...
const EditAgent = ({ setShowEditAgent, data, agentId, fetchAgents }) => {
    ...
    useEffect(() => {
        if(data?.id && data?.languages) {
            const json = data?.languages ? JSON.parse(data?.languages) : '';
            const language = json && json?.split(',');
            // console.log('lang: ', language);
            setLanguages(languagesData.map(item => language.includes(item.title) ? {...item, selected: true} : item));

            setFields(prev => ({ ...prev, orgName: data?.name, languages: language }));
            setPreview(prev => ({...prev, orgImage: data?.image}));
        }
    }, [data?.id, data?.languages])
    ...
    const handleSubmit = async () => {
        // if (!fields?.orgImage || !fields?.orgName || !fields?.languages) return message.error("Please fill all fields and upload image!");
        if (!fields?.orgName || !fields?.languages) return message.error("Please fill all fields and upload image!");
        const formData = new FormData();
        fields?.orgImage && formData.append("orgImage", fields?.orgImage);
        ...
        try {
            const response = await axiosClient.patch("/agents", formData, {
                headers: {
                  "Content-Type": "multipart/form-data",
                },
            });
            console.log('res.data in /update-agent: ', response?.data);
            message.success('Organization edited successfully!');
            await fetchAgents();
            setShowEditAgent(false);
        } catch (error) {
          console.error("Error in /update-agent: ", error?.response?.data || error?.message || error);
          ...
          message.error("There is an error while 'updating agent': "+msg);
        }
    };
                ...
                <div className="box relative w-full py-[3rem] pb-[2rem] xs:pb-[3rem] border-[1px] border-[rgba(255,255,255,0.30)] backdrop-blur-[40px] rounded-[1.5rem] z-50" style={{ background: 'linear-gradient(90deg, rgba(96, 92, 241, 0.10) 0%, rgba(160, 57, 252, 0.10) 100%)', boxShadow: 'rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.23) 0px 6px 6px'}}>
                    <div className="custom-scroll w-[98%] 3xs:w-[95%] sm:w-[85%] md:w-[80%] max-h-[85vh] px-[1rem] overflow-y-auto mx-auto flex flex-col gap-[.7rem] xs:gap-[1.2rem]">
                        <div className="title-section text-center">
                            <div className="title text-[1.7rem] xs:text-[1.9rem] 3xl:text-[2.25rem] leading-[1.2] text-[#EFEFEF] font-semibold">Edit {fields?.orgName} Organization</div>
                            <div className="sub-title text-[1rem] text-[#A4A4A4] font-medium mt-[3px]">Enter the following details to edit agent</div>
                        </div>
                        ...
                        <div className="input-section flex flex-col gap-[.5rem] xs:gap-[.75rem] text-[#A4A4A4] font-medium">
                            ...
                            <div className="input-container">
                                ...
                                <div className="input px-[1.125rem] flex items-center gap-[1.25rem] text-[.95rem] backdrop-blur-[6px] border-[0.3px] border-[rgba(255,255,255,.25)] rounded-[.5rem] mt-[.35rem] xs:mt-[.5rem]" style={{ background: 'radial-gradient(151.92% 127.02% at 15.32% 21.04%, rgba(255, 255, 255, 0.20) 0%, rgba(255, 255, 255, 0.04) 77.08%, rgba(255, 255, 255, 0.00) 100%)' }}>
                                    <div className="icon"><img src="/assets/Login/user-icon.svg" alt="" /></div>
                                    <input type="text" placeholder="•••••••••" className="w-full py-[.75rem] xs:py-[.95rem] bg-transparent outline-none" id="password-input" name="password" value={fields?.password} onChange={handleInputChange} required />
                                </div>
                            </div>
                        ...
                        <div className="button py-[.6rem] xs:py-[.75rem] bg-[#EF0B64] text-[1.05rem] xs:text-[1.125rem] text-center font-medium rounded-[.9375rem] mt-[.3rem]" onClick={handleSubmit}>Update Organization</div>
                    </div>
                    <div className="button absolute top-[.6rem] right-[.6rem] md:top-[1rem] md:right-[1rem]" onClick={() => setShowEditAgent(false)}><img src="/assets/Agents/cross.svg" className="w-[2.1rem] md:w-[2.7rem]" alt="" /></div>
                </div>
            </div>
        </div>
    )
