...
const EditUser = ({ setShowEditUser, data, fetchUsers }) => {
    ...
    useEffect(() => {
        if(data?.id && data?.name) {
            setFields(prev => ({ ...prev, name: data?.name, email: data?.email }));
    ...
    const handleSubmit = async () => {
        if (!fields?.email || !fields?.name) return message.error("Please fill all fields and upload image!");
        ...
        try {
            const response = await axiosClient.patch("/users", formData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                },
            });
            console.log('res.data in /edit-user: ', response?.data);
            message.success('User edited successfully!');
            await fetchUsers();
            setShowEditUser(false);
        } catch (error) {
            console.error("Error in /edit-user: ", error?.response?.data || error?.message || error);
            ...
            message.error("There is an error while 'editing user': ", msg);
        }
    };
                ...
                <div className="box relative w-full py-[3rem] pb-[1.75rem] xs:pb-[3rem] border-[1px] border-[rgba(255,255,255,0.30)] backdrop-blur-[40px] rounded-[1.5rem] z-50" style={{ background: 'linear-gradient(90deg, rgba(96, 92, 241, 0.10) 0%, rgba(160, 57, 252, 0.10) 100%)', boxShadow: 'rgba(0, 0, 0, 0.19) 0px 10px 20px, rgba(0, 0, 0, 0.23) 0px 6px 6px'}}>
                    <div className="w-[90%] sm:w-[85%] md:w-[80%] mx-auto flex flex-col gap-[.75rem] xs:gap-[1rem]">
                        <div className="title-section text-center">
                            <div className="title text-[1.7rem] xs:text-[1.9rem] 3xl:text-[2.25rem] leading-[1.15] text-[#EFEFEF] font-semibold">Edit {fields?.name} User</div>
                            <div className="sub-title text-[.9rem] xs:text-[1rem] text-[#A4A4A4] font-medium mt-[3px]">Enter the following details to edit user</div>
                        </div>
                        <div className="input-section flex flex-col gap-[.6rem] xs:gap-[.75rem] text-[#A4A4A4] font-medium">
                            <div className="input-container">
                                ...
                                <div className="input px-[1.125rem] flex items-center gap-[1.25rem] text-[.95rem] backdrop-blur-[6px] border-[0.3px] border-[rgba(255,255,255,.25)] rounded-[.5rem] mt-[.3rem] xs:mt-[.5rem]" style={{ background: 'radial-gradient(151.92% 127.02% at 15.32% 21.04%, rgba(255, 255, 255, 0.20) 0%, rgba(255, 255, 255, 0.04) 77.08%, rgba(255, 255, 255, 0.00) 100%)' }}>
                                    ...
                                    <input type="text" placeholder="Email" disabled className="w-full py-[.7rem] sm:py-[.95rem] text-gray-500 bg-transparent outline-none" id="email-input" name="email" value={fields?.email} onChange={handleInputChange} required />
                                </div>
                            </div>
                            {/* <div className="input-container">
                                ...
                                <div className="input px-[1.125rem] flex items-center gap-[1.25rem] text-[.95rem] backdrop-blur-[6px] border-[0.3px] border-[rgba(255,255,255,.25)] rounded-[.5rem] mt-[.3rem] xs:mt-[.5rem]" style={{ background: 'radial-gradient(151.92% 127.02% at 15.32% 21.04%, rgba(255, 255, 255, 0.20) 0%, rgba(255, 255, 255, 0.04) 77.08%, rgba(255, 255, 255, 0.00) 100%)' }}>
                                    ...
                                    <input type="text" placeholder="•••••••••" className="w-full py-[.7rem] sm:py-[.95rem] bg-transparent outline-none" id="password-input" name="password" value={fields?.password} onChange={handleInputChange} required />
                                </div>
                            </div> */}
                        ...
                        <div className="flex justify-between items-center">
                            <div className="text-[.85rem] xs:text-[.95rem] text-[#A4A4A4] font-medium mt-[3px]"><Link href={'/change-password'} className="text-gray-200 underline">Change your Password</Link></div>
                            <div className="text-[.85rem] xs:text-[.95rem] text-[#A4A4A4] font-medium mt-[3px]"><Link href={'/forget-password'} className="text-gray-200 underline">Reset Password</Link></div>
                        </div>
                        <div className="button py-[.6rem] xs:py-[.75rem] bg-[#EF0B64] text-[1.05rem] xs:text-[1.125rem] text-center font-medium rounded-[.9375rem] mt-[.3rem]" onClick={handleSubmit}>Edit User</div>
                    </div>
                    <div className="button absolute top-[.6rem] right-[.6rem] md:top-[1rem] md:right-[1rem]" onClick={() => setShowEditUser(false)}><img src="/assets/Agents/cross.svg" className="w-[2.1rem] md:w-[2.7rem]" alt="" /></div>
                </div>
            </div>
        </div>
    )
